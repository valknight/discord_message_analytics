...
insert_role = "INSERT INTO `gssp`.`roles` (`role_id`, `role_name`, `guild_id`, `is_joinable`) VALUES (%s, %s, %s, %s);"
update_role = "UPDATE `gssp`.`roles` SET `role_name`=%s, `is_joinable`=%s WHERE `role_id`=%s;" # we don't have guild_id here, as this doesn't change.
